...
const {handleShowSidebar,showSidebar} = useGlobalContext();
  ...
  className="cursor-pointer p-1 h-8 w-8 sm:hidden hover:scale-110"
>
  <AlignJustify className="w-full h-full" />
          ...
          <div className="hidden sm:flex   sm:text-lg md:text-xl items-center justify-center  space-x-6">
            ...
            <Link to='/auth' className='text-gray-700 border-none rounded-sm px-3 font-semibold text-md py-1 hover:text-white hover:bg-blue-500 '>Login</Link>
          </div>
</div>     </div>
    </nav>
  );
